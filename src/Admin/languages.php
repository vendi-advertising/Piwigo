...
;';
    ...
    function get_fs_languages($target_charset = null)
    {
        ...
        while ($file = readdir($dir)) {
            if ($file !== '.' && $file !== '..') {
                $path = Path::join(PHPWG_ROOT_PATH, 'language', $file);
                if (is_dir($path) && !is_link($path) && preg_match('/^[a-zA-Z0-9-_]+$/', $file) && file_exists($path.'/common.lang.php')) {
                    ...
                    // IMPORTANT SECURITY !
                    // TODO: This should be handled on render
    ...
    {
        $em = App\PWKernel::getInstance()->getEntityManager();

        $languageRepository = $em->getRepository(Language::class);
        $languages = $languageRepository->findAll();
        /** @var Language $language */
        foreach ($languages as $language) {
            $this->db_languages[$language->getId()] = $language->getName();
        }
//        $query = '
//  SELECT id, name
//    FROM '.LANGUAGES_TABLE.'
